...
type storeFactory func(*testing.T) wsession.Store
...
func GetSet(t *testing.T, newStore storeFactory) {
	r := gin.Default()
	r.Use(wsession.New(sessionName, newStore(t)))
...
func Flashes(t *testing.T, newStore storeFactory) {
	r := gin.Default()
	store := newStore(t)
	r.Use(wsession.New(sessionName, store))
...
func Options(t *testing.T, newStore storeFactory) {
	r := gin.Default()
	store := newStore(t)
	store.Options(wsession.Options{
		Domain: "localhost",
	})
	r.Use(wsession.New(sessionName, store))
	...
	r.GET("/domain", func(c *gin.Context) {
		session := wsession.Default(c)
		session.Set("key", ok)
		session.Options(wsession.Options{
			Path: "/foo/bar/bat",
		})
		_ = session.Save()
		c.String(http.StatusOK, ok)
...
func Many(t *testing.T, newStore storeFactory) {
	...
	r.Use(wsession.NewMany(sessionNames, newStore(t)))
